...
def write_brief(out, cfg, sub, gen, rows, report, error=None):
    ...
    if mode == "params":
        ...
        parts.append("ACTION SPACE\nYou write the complete C++17 source of a strategy plugin: one file that "
                     ...
                     "YOU OWN THE WHOLE STACK, NOT JUST THE DECISION\n"
                     "You run on your own thread. How you receive market data, how you process it, and how you "
                     "publish orders are all yours to choose. The defaults are defaults, not requirements.\n"
                     "- Strategy::transport() picks the plumbing between you and the engine. Return\n"
                     "  TransportKind::Spsc for a lock-free single-producer ring that you poll,\n"
                     "  TransportKind::Mutex for a mutex-guarded ring that you poll, or\n"
                     "  TransportKind::MutexCv for a mutex plus condition variable where your thread parks\n"
                     "  until an event arrives and the wake costs a trip through the kernel.\n"
                     "  Return std::nullopt to accept the host default. These differ in what they cost you\n"
                     "  in the typical case and in the tail, and the right answer depends on your design and\n"
                     "  on how loaded the machine is.\n"
                     "- Strategy::run(ctx, running) is your event loop. The default drains up to 256 events, "
                     "then calls on_idle, then waits. Override it and the batch size, how you wait, when you do "
                     "housekeeping and when you submit are all under your control. If you override run(), you "
                     "must call ctx.apply(ev) for every event you consume or your own position and cash will "
                     "drift from the engine's, and you must return once `running` reads false.\n"
                     "- If you keep the default run(), on_event and on_idle must return quickly: no sleeping, "
