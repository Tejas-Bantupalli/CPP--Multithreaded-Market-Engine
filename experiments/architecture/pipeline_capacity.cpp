// Capacity-only extension of pipeline_balance.cpp; same stage work and timing.
#include "common.h"
#include "queues.h"
#include <string>
...
template<size_t CAP> int run_case(int argc,char**argv){
    try{
        ...
        auto q=make_queue<Message,CAP>(128,false);
        ...
        std::cout<<"{\"correct\":"<<(correct?"true":"false")<<",\"count\":"<<count<<",\"first\":"<<first<<",\"second\":"<<second
                 <<",\"usable_capacity\":"<<CAP-1<<",\"topology\":\""<<topology<<"\",\"elapsed_ns\":"<<finish-epoch<<",\"producer_cpu_ns\":"<<pcpu<<",\"consumer_cpu_ns\":"<<ccpu<<",\"latency_ns\":";
...
int main(int argc,char**argv){
    if(argc<2)return 1;
    int capacity;
    try{capacity=std::stoi(argv[1]);}catch(...){return 1;}
    switch(capacity){
        case 2:return run_case<2>(argc-1,argv+1);
        case 16:return run_case<16>(argc-1,argv+1);
        case 128:return run_case<128>(argc-1,argv+1);
        case 1024:return run_case<1024>(argc-1,argv+1);
        default:return 1;
    }
}
